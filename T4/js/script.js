...
ajax('backend/getEtapas.php', 'GET', (response) => {
  titulos = JSON.parse(response)
  titulos.sort((a, b) => {
    return   b.numeros-a.numeros;
  });
  ajax('backend/getCandidatos.php', 'GET', (response) => {
    candidatos = JSON.parse(response)
    
    etapas=[]
    for (t of titulos) {
      candidatosPorTitulo = candidatos.filter(c => c.titulo == t.titulo)
      obj = {
        "titulo": t.titulo,
        "numeros": t.numeros,
        "candidatos": []
    ...
    for (c of candidatosPorTitulo) {
      ...
      if (c.Vice == null) {
        ...
        candidato = {
          ...
          "vice": {
            "nome": c.Vice,
            "partido": c.VicePartido,
            "foto": c.ViceFoto
          }
        }
      }
      obj["candidatos"].push(candidato)
...
function atualizarInterface() {
  ...
  let candidato = null
  candidatofilter = etapa['candidatos'].filter(c=> c.numero==numeroDigitado)
  if(candidatofilter.length>0){
      candidato = candidatofilter[0]
